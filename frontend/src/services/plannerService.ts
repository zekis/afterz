...
export class PlannerService {
  ...
  static async getPlannerEntries(startDate: Date, endDate: Date, user?: string): Promise<PlannerEntry[]> {
    const res = await FrappeAPI.post<PlannerEntry[]>('afterz.beforez_api.get_planner_entries', {
  ...
  static async createPlannerEntry(entry: Partial<PlannerEntry>): Promise<{ success: boolean; name?: string; error?: string }> {
    const res = await FrappeAPI.post<any>('afterz.beforez_api.create_planner_entry', entry)
    return res.message
  }

  static async updatePlannerEntry(name: string, updates: Partial<PlannerEntry>): Promise<{ success: boolean; error?: string }> {
    const res = await FrappeAPI.post<any>('afterz.beforez_api.update_planner_entry', {
      name,
      ...updates
    })
    return res.message
  ...
  static async deletePlannerEntry(name: string): Promise<{ success: boolean; error?: string }> {
    const res = await FrappeAPI.post<any>('afterz.beforez_api.delete_planner_entry', { name })
    return res.message
  }

  static async completeEntry(name: string): Promise<{ success: boolean; error?: string }> {
    const res = await FrappeAPI.post<any>('afterz.beforez_api.complete_planner_entry', { name })
    return res.message
  }

  static async cancelEntry(name: string): Promise<{ success: boolean; error?: string }> {
    const res = await FrappeAPI.post<any>('afterz.beforez_api.cancel_planner_entry', { name })
    return res.message
  }

  static async getUserTodos(user?: string): Promise<TodoLite[]> {
    const res = await FrappeAPI.post<TodoLite[]>('afterz.beforez_api.get_user_todos', { user })
    return res.message || []
  }
}
